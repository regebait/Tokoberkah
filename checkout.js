...
router.post("/", requireAuth, requireRole("buyer"), (req, res) => {
  ...
  if (!Array.isArray(items) || items.length === 0 || !paymentMethod) {
    return res.status(400).json({ error: "Keranjang kosong atau metode pembayaran belum dipilih." });
  }
  ...
  const tx = db.transaction(() => {
    for (const item of items) {
      ...
      if (product.stok < item.qty) throw new Error(`Stok ${product.name} tidak cukup.`);

      db.prepare("UPDATE products SET stok = stok - ? WHERE id = ?").run(item.qty, product.id);
  ...
  ).run(invoiceNumber, req.user.username, JSON.stringify(invoiceItems), total, paymentMethod);
