...
router.post("/", requireAuth, requireRole("seller"), (req, res) => {
  ...
  if (!name || !jenis || harga == null || stok == null || harga < 0 || stok < 0) {
    return res.status(400).json({ error: "Lengkapi nama, jenis, harga, dan stok dengan benar." });
  }
  const info = db
    .prepare("INSERT INTO products (name, jenis, harga, stok, icon, seller_id) VALUES (?, ?, ?, ?, ?, ?)")
    .run(name, jenis, harga, stok, icon || "📦", req.user.id);
